...
この章では、映画の一覧と詳細を分けます。読了後には、`/movies` を映画の集合、`/movies/:id` を 1 件の映画として扱い、ID で映画を探して詳細画面を表示できるようになります。
...
映画が見つからなかったときは、`halt 404, "映画が見つかりません"` で処理を止めます。`halt` は、ここで Sinatra の処理を止め、指定したステータスコードと本文をレスポンスとして返します。ここでは専用の 404 ページはまだ作りません。存在しない ID に対して 404 のレスポンスを返すところまでを扱います。404 ページは第10章で作ります。
...
- `/movies` と `/movies/:id` は、どちらも映画に関係する URI ですが、何が違うのでしょうか。
