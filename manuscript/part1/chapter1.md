...
ところが、本書で作る映画図鑑では、映画を探す処理や保存する処理は Ruby のプログラムが担当します。ブラウザと Ruby のプログラムは、どのようにつながるのでしょうか。

**この章のゴール:** ブラウザ、サーバー、リクエスト、レスポンスの関係を、図と Chrome DevTools の Network パネルに表示される情報を使って説明できる。
...
`200 OK` は**ステータスコード**と呼ばれ、リクエストが成功したことを表します。`Content-Type` ヘッダーは、レスポンス本文がどの種類のデータであるかを表します。空行より後ろの `<h1>映画図鑑</h1>` がレスポンスの本文です。
...
HTTP には、HTTP/1.1、HTTP/2、HTTP/3 という複数のバージョンがあります。通信の効率や内部表現には違いがありますが、リクエストとレスポンスをやり取りし、メソッドやステータスコードで意味を伝える基本は共通しています。

この章では、人が構造を追いやすい HTTP/1.1 の例を使いました。各バージョンの違いを学ぶときも、まずメソッド、パス、ステータスコード、ヘッダー、本文のどれについての違いなのかを分けて読むと、現在学んでいる内容との関係を見失いにくくなります。
