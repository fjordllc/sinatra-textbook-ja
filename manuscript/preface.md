...
Sinatra は、小さなフレームワークです。`get "/movies" do ... end` と書けば、「`GET /movies` というリクエストに、この処理で応える」という対応が、そのままコードに表れます。URL と HTTP メソッド、そして Ruby の処理の結び付きが、余計な仕組みに隠れず目に見えます。

また、Sinatra のアプリは小さく保てます。本書の映画図鑑は、基本的に一つの `app.rb` に収まります。アプリ全体を一度に見渡せるので、一つのリクエストが届いてからレスポンスが返るまでを、最初から最後まで自分で追えます。これは、Network パネルで実際の通信を見ながら進める本書のやり方と、よく合います。
