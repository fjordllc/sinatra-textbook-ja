...
Sinatra 側では、Rack の method override を有効にします。`app.rb` の `require` の並びより後、`MOVIES_FILE` より前へ置きます。
...
まず、読み込んだ配列の中から映画を探すメソッドを追加します。`find_movie` の下、`movie_params` より前へ置きます。
...
`PATCH /movies/:id` を追加します。ルートの並びでは、`post "/movies"` の下、後で追加する `delete "/movies/:id"` より前へ置きます。
...
削除は、詳細画面にフォームを置いて行います。HTML フォームは DELETE を直接送れません。そのため、フォームの `method` は `post` にし、hidden input で `_method=delete` を送ります。
...
`DELETE /movies/:id` を追加します。`patch "/movies/:id"` の下へ置きます。
