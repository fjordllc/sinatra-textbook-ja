...
この章では、映画を登録するためのフォームを作ります。ここでの目的は、まだ映画を保存することではありません。

フォームの HTML と実際の通信を見比べます。どの URL へ、どの HTTP メソッドで送るのかを確認します。さらに、入力した値がどのキーで Sinatra の `params` に届くのかを追います。
