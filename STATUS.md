...
付録Aから付録Eの執筆と、全体の横断レビュー、レビュー統合、修正、再確認が完了しました。第1章から第12章の本編、付録、サンプルアプリコードは最終検証へ進める状態です。次は付録・横断レビュー分をコミットし、サンプルアプリのランタイム確認を含む最終検証へ進みます。
...
1. 付録・横断レビュー分をコミットし、公開リポジトリへプッシュする。
2. サンプルアプリのランタイム確認を含む最終検証を行う。
3. 最終検証結果を記録し、最終コミットとタグを作成する。

## レビューへの対応状況

第1章から第12章、付録Aから付録E、横断レビューは完了済みです。横断レビューの `review-summary.md`、`changes.md`、`second-review.md` を保存しました。未解決の `must` はなく、採用した `should` は修正済みです。
