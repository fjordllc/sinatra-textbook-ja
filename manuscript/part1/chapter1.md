...
この章では、まだコードを書きません。第2章で Sinatra を起動し、最初のリクエストを受け取るところから始めます。

その前に、ブラウザとサーバーの役割の違いを押さえておきます。すると、Sinatra の `get` が「どのリクエストに、どのレスポンスを返すか」を決めるコードだと読み取れるようになります。
