...
この章では、これまで使ってきたリダイレクトを PRG パターンとして捉え直します。読了後には、状態を変えるリクエストの後に、なぜ直接 HTML を返さずリダイレクトするのかを説明できるようになります。
...
この環境では、ステータスコードとして `303 See Other` を確認できます。`303 See Other` は、別の URI を GET で取りに行くよう示すリダイレクトです。
